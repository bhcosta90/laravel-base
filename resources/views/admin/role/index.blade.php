...
@section('content')
    @if(!empty($filter))
        <div class='card'>
            <div class='card-header'>
                <div class='float-left'>{{ __('Filtrar Grupos') }}</div>
    ...
    <div class='card'>
        <div class='card-header'>
            <div class='float-left'>{{ __('Relatório de Grupos') }}</div>
            @empty(!$linkRegister)
                <div class='float-right'><a href="{{ $linkRegister }}" class='btn btn-light'>{{__('Cadastrar Grupo')}}</a></div>
            @endif
        </div>
